...
2. Configura:
   ...
   - **Hadoop Configuration Resources** (o Controller Service): apunta a tu `core-site.xml` y `hdfs-site.xml`.
   - **Apply**.
...
1. Selecciona los procesadores (para GPS: GetFile y PublishKafka filtered-data).
2. Clic derecho → **Start**.
3. Comprueba: `kafka-console-consumer.sh --bootstrap-server localhost:9092 --topic filtered-data --from-beginning --max-messages 3`
...
Opcional: GetFile → PutHDFS → `/user/hadoop/raw`. Otros: InvokeHTTP OpenWeather/FlightRadar → PublishKafka → raw-data.

---

## Referencias

- [NIFI_FLUJOS.md](../guides/NIFI_FLUJOS.md)
- [FUENTES_DATOS.md](../guides/FUENTES_DATOS.md)
